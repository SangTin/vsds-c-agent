...
def main(rebuild_index: bool = False):
    """v25 final config: CoT 600t (incl. passage) + targeted RAG + alignment shortcut.

    Reproduces public-test leaderboard 84.67. Same config as Dockerfile.
    Experiment branches in git preserve dropped variants.
    """
    ...
    out = Path("../output/pred.csv")
    out.parent.mkdir(parents=True, exist_ok=True)
    out.write_text(pred, encoding="utf-8")
    newline = chr(10)
    rows = pred.strip().count(newline)
